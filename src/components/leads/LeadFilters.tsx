...
const LeadFilters: React.FC<LeadFiltersProps> = ({
  ...
  const sourceOptions = [
    'Instagram', 'Facebook', 'YouTube', 'Reference', '99acres', 'MagicBricks', 'Direct', 'Website', 'Social Media', 'Advertisement', 'Other'
  ];
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.filtersContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.chipContainer}>
            {statusOptions.map(option => (
              <FilterChip
                ...
                onPress={() => onFilterChange('status', filters.status === option.value ? '' : option.value)}
              />
            ))}
          </View>
        </FilterSection>
          ...
          <View style={styles.chipContainer}>
            {sourceOptions.map(source => (
              <FilterChip
                ...
                onPress={() => onFilterChange('source', filters.source === source ? '' : source)}
              />
            ))}
          </View>
        </FilterSection>
          ...
          <View style={styles.chipContainer}>
            {budgetRanges.map(range => (
              <FilterChip
                ...
                value={range}
                isSelected={filters.budget === range}
                onPress={() => onFilterChange('budget', filters.budget === range ? '' : range)}
              />
            ))}
          </View>
        </FilterSection>
        ...
        {/* Assignment Filter */}
        <FilterSection title="Assignment" section="assignedTo">
          <View style={styles.chipContainer}>
            {assignmentOptions.map(option => (
              <FilterChip
                ...
                label={option.label}
                value={option.value}
                isSelected={filters.assignedTo === option.value}
                onPress={() => onFilterChange('assignedTo', filters.assignedTo === option.value ? '' : option.value)}
              />
            ))}
          </View>
        </FilterSection>
            ...
            <View style={styles.chipContainer}>
              ...
              {availableUsers.map(user => (
                <FilterChip
                  ...
                  onPress={() => onFilterChange('createdBy', filters.createdBy === (user.id || user.userId)?.toString() ? '' : (user.id || user.userId)?.toString())}
                />
              ))}
            </View>
          </FilterSection>
